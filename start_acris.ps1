...
Start-Process ".\redis-server.exe" -ArgumentList "redis.windows.conf" -WindowStyle Hidden

# 2. Start Celery Worker
Write-Host "[2/4] Starting Celery Worker..." -ForegroundColor Yellow
Start-Process "python" -ArgumentList "start_celery_worker.py" -NoNewWindow
...
Start-Process "pwsh" -ArgumentList "-File start_frontend.ps1" -NoNewWindow
